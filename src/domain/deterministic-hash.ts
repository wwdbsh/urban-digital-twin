/** Canonical key-sorted serialization shared by deterministic domain plans. */
...
export function sha256HexSync(value: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    for (let index = 0; index < 16; index += 1) { const start = offset + index * 4; words[index] = ((padded[start]! << 24) | (padded[start + 1]! << 16) | (padded[start + 2]! << 8) | padded[start + 3]!) >>> 0; }
    for (let index = 16; index < 64; index += 1) { const valueA = words[index - 15]!; const valueB = words[index - 2]!; const sigma0 = rotateRight(valueA, 7) ^ rotateRight(valueA, 18) ^ (valueA >>> 3); const sigma1 = rotateRight(valueB, 17) ^ rotateRight(valueB, 19) ^ (valueB >>> 10); words[index] = (words[index - 16]! + sigma0 + words[index - 7]! + sigma1) >>> 0; }
    let [a, b, c, d, e, f, g, h] = hash;
    for (let index = 0; index < 64; index += 1) { const sigma1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25); const choose = (e & f) ^ (~e & g); const temp1 = (h + sigma1 + choose + SHA256_K[index]! + words[index]!) >>> 0; const sigma0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22); const majority = (a & b) ^ (a & c) ^ (b & c); const temp2 = (sigma0 + majority) >>> 0; h = g; g = f; f = e; e = (d + temp1) >>> 0; d = c; c = b; b = a; a = (temp1 + temp2) >>> 0; }
...
/** Hashes payloads in explicit namespaces so unlike contracts cannot collide. */
export function domainSeparatedSha256(namespace: string, value: unknown): string {
  return sha256HexSync(`${namespace}\n${stableSerialize(value)}`);
}
