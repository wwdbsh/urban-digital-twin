...
function withdrawingRollback(record: ExteriorDefaultActivationRecord): ExteriorDefaultActivationRecord {
  ...
  const shipped = EXTERIOR_SERVING_ROLLBACKS.find((entry) => entry.rolledBackReleaseId === record.releaseId);
...
const ROLLED_BACK_TO_PREDECESSOR: ExteriorDefaultActivationRecord = withdrawingRollback(EXTERIOR_DEFAULT_ACTIVATION);
 ...
 */
const MIDTOWN_ROLLED_BACK: ExteriorDefaultActivationRecord = withdrawingRollback(MIDTOWN_CORE_EXTERIOR_ACTIVATION);
 ...
 */
const LOWER_MANHATTAN_ROLLED_BACK: ExteriorDefaultActivationRecord = withdrawingRollback(LOWER_MANHATTAN_EXTERIOR_ACTIVATION);
/**
 ...
 */
const LOWER_MANHATTAN_ROLLED_BACK_TO_BASE: ExteriorDefaultActivationRecord = LOWER_MANHATTAN && LOWER_MANHATTAN.predecessor.enabled
  ? LOWER_MANHATTAN.predecessor.predecessor
  : LOWER_MANHATTAN_EXTERIOR_ACTIVATION;
 ...
 */
const SOUTHERN_REMAINDER_ROLLED_BACK: ExteriorDefaultActivationRecord = withdrawingRollback(SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION);
 ...
 */
const CENTRAL_UPPER_MANHATTAN_ROLLED_BACK: ExteriorDefaultActivationRecord = withdrawingRollback(CENTRAL_UPPER_MANHATTAN_EXTERIOR_ACTIVATION);
 ...
 * other wave.
 */
const NORTHERN_MANHATTAN_ROLLED_BACK: ExteriorDefaultActivationRecord = withdrawingRollback(NORTHERN_MANHATTAN_EXTERIOR_ACTIVATION);
/**
...
const SERVING_INVENTORY = JSON.parse(
  new TextDecoder().decode(readFileSync(`data/${BLOCK835_SERVING_RELEASE_ID}/payload-inventory.json`)),
...
describe("the promoted set", () => {
  it("orders all SIX declared waves, Block 835 first and Northern-Manhattan last, from the records the build exports", () => {
    ...
    expect(exteriorDefaultActivations(ROLLED_BACK)).toEqual([ROLLED_BACK, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION, CENTRAL_UPPER_MANHATTAN_EXTERIOR_ACTIVATION, NORTHERN_MANHATTAN_EXTERIOR_ACTIVATION]);
    expect(exteriorDefaultActivations(EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_ROLLED_BACK)).toEqual([EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_ROLLED_BACK, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION, CENTRAL_UPPER_MANHATTAN_EXTERIOR_ACTIVATION, NORTHERN_MANHATTAN_EXTERIOR_ACTIVATION]);
    expect(exteriorDefaultActivations(EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_ROLLED_BACK))
      .toEqual([EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_ROLLED_BACK, SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION, CENTRAL_UPPER_MANHATTAN_EXTERIOR_ACTIVATION, NORTHERN_MANHATTAN_EXTERIOR_ACTIVATION]);
    expect(exteriorDefaultActivations(EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_ROLLED_BACK))
      .toEqual([EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_ROLLED_BACK, CENTRAL_UPPER_MANHATTAN_EXTERIOR_ACTIVATION, NORTHERN_MANHATTAN_EXTERIOR_ACTIVATION]);
    expect(exteriorDefaultActivations(EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION, CENTRAL_UPPER_MANHATTAN_ROLLED_BACK))
      .toEqual([EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION, CENTRAL_UPPER_MANHATTAN_ROLLED_BACK, NORTHERN_MANHATTAN_EXTERIOR_ACTIVATION]);
    expect(exteriorDefaultActivations(EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION, CENTRAL_UPPER_MANHATTAN_EXTERIOR_ACTIVATION, NORTHERN_MANHATTAN_ROLLED_BACK))
      .toEqual([EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_EXTERIOR_ACTIVATION, SOUTHERN_REMAINDER_EXTERIOR_ACTIVATION, CENTRAL_UPPER_MANHATTAN_EXTERIOR_ACTIVATION, NORTHERN_MANHATTAN_ROLLED_BACK]);
  });
  ...
  it("promotes disjoint cell ids, so no wave can claim another wave's cell", () => {
    ...
    for (const record of EXTERIOR_DEFAULT_ACTIVATIONS) {
      ...
      for (const entry of [record, record.predecessor]) {
  ...
  it("keeps the partial-rollback impossibility PER RECORD", () => {
    for (const record of EXTERIOR_DEFAULT_ACTIVATIONS) {
      ...
      if (record.predecessor.enabled) {
        ...
        // Every enabled predecessor in this build is a curated record, and the
        // rule that a rollback target NAMES the release it withdraws still holds
        // of those records' own rollback targets — one link further down the
        // chain. What it no longer holds of is the serving promotion's own
        // predecessors; that fall is asserted on its own below.
        const curatedRollback = record.predecessor.predecessor;
        expect(curatedRollback.rolledBackReleaseId, record.predecessor.releaseId).toBe(record.predecessor.releaseId);
      } else {
   ...
   */
  it("refuses a withdrawn -s1 bookmark on every wave, while the restored curated link still opens", () => {
    for (const record of EXTERIOR_DEFAULT_ACTIVATIONS) {
      if (!record.enabled) continue;
      const rollback = withdrawingRollback(record);
  ...
  it("cannot withdraw two releases in one record, which is why a deeper rollback is two swaps", () => {
    for (const record of EXTERIOR_DEFAULT_ACTIVATIONS) {
      ...
      // The second swap already exists: the curated record's own predecessor,
      // which names the curated release as withdrawn.
      expect(rollback.predecessor.rolledBackReleaseId, rollback.releaseId).toBe(rollback.releaseId);
    }
  });
});
...
describe("behaviour neutrality of the set resolution over one record", () => {
  ...
  it("keeps the refusal, restore, and verification rules identical for one record", async () => {
    ...
    const failure = verifyPromotedExteriorMembership(["doitt:999999"], EXTERIOR_DEFAULT_ACTIVATION, BLOCK835_MEMBERSHIP_BUILDING_IDS);
...
describe("the promoted set as this build actually ships it", () => {
  ...
  it("streams all six promoted waves over a real base, each gated by its own record", () => {
    ...
    expect(set.targets[5]!.record).toBe(NORTHERN_MANHATTAN_EXTERIOR_ACTIVATION);
    // The URL still serialises nothing: a default-on session's links stay
    // reproducible against whatever the build promotes.
    expect(set.primaryReleaseId).toBe(BLOCK835_SERVING_RELEASE_ID);
  });
  ...
  it("rehearses the Block 835 serving rollback and roll-forward with the other five untouched", () => {
    ...
    // The withdrawn serving link fails closed, by name, and only for Block 835.
    const refused = resolveExteriorActivationSet({ ...base, override: "on", explicitReleaseId: BLOCK835_SERVING_RELEASE_ID, activeRealBaseReleaseId: CITYWIDE_BASE, records: rolledBack });
    expect(refused.streaming).toBe(false);
    expect(refused.releases[0]!.reason).toBe("rolled-back-release");
    expect(exteriorRolledBackReleaseNotice(MIDTOWN_SERVING_RELEASE_ID, rolledBack)).toBeNull();
    // The restored curated release's own link is not refused by the swap.
  ...
  it("rolls the Midtown serving wave back to its curated V3 predecessor without withdrawing Block 835", () => {
    ...
    const refused = resolveExteriorActivationSet({ ...base, override: "on", explicitReleaseId: MIDTOWN_SERVING_RELEASE_ID, activeRealBaseReleaseId: CITYWIDE_BASE, records: midtownRolledBack });
    expect(refused.streaming).toBe(false);
    expect(refused.releases[0]!.reason).toBe("rolled-back-release");
    expect(exteriorRolledBackReleaseNotice(MIDTOWN_SERVING_RELEASE_ID, midtownRolledBack))
      .toContain(`${MIDTOWN_SERVING_RELEASE_ID} was rolled back in this build`);
    // Block 835's link is untouched by the Midtown withdrawal, and so is the
    // restored curated V3 link.
    expect(exteriorRolledBackReleaseNotice(BLOCK835_SERVING_RELEASE_ID, midtownRolledBack)).toBeNull();
  ...
  it("rolls the Lower-Manhattan wave back to its curated P1 release without withdrawing the other five", () => {
    ...
    const refused = resolveExteriorActivationSet({ ...base, override: "on", explicitReleaseId: LOWER_MANHATTAN_SERVING_RELEASE_ID, activeRealBaseReleaseId: CITYWIDE_BASE, records: rolledBack });
    expect(refused.streaming).toBe(false);
    expect(refused.releases[0]!.reason).toBe("rolled-back-release");
    expect(exteriorRolledBackReleaseNotice(LOWER_MANHATTAN_SERVING_RELEASE_ID, rolledBack))
      .toContain(`${LOWER_MANHATTAN_SERVING_RELEASE_ID} was rolled back in this build`);
  ...
  /**
   * The base-massing rollback, which is now the SECOND swap rather than the
   * first.
   *
   * Before the serving promotion, four waves rolled back to base massing in one
   * edit. None does now, so the case that proves a wave going fully dark leaves
   * the others streaming has to reach base the way a build actually would:
   * export the curated record's own base-only predecessor, which is the record
   * this build already ships for that withdrawal.
   *
   * It refuses the CURATED link, not the serving one, because
   * `rolledBackReleaseId` names exactly one release. Withdrawing both is two
   * statements and therefore two swaps; that limit is a property of the record
   * shape and is stated here rather than papered over.
   */
  it("rolls the Lower-Manhattan wave back twice, all the way to BASE MASSING, with the other five still streaming", () => {
    const rolledBack = exteriorDefaultActivations(EXTERIOR_DEFAULT_ACTIVATION, MIDTOWN_CORE_EXTERIOR_ACTIVATION, LOWER_MANHATTAN_ROLLED_BACK_TO_BASE);
    const set = resolveExteriorActivationSet({ ...base, override: null, explicitReleaseId: null, activeRealBaseReleaseId: CITYWIDE_BASE, records: rolledBack });
    ...
    // And the serving link is NOT refused by this record: withdrawing it is the
    // first swap's statement, which this record does not carry.
  ...
  it("rolls the Southern-remainder wave back to its curated P1 release without withdrawing the other five", () => {
    ...
    // The withdrawn opt-in link is refused BY NAME, in the same one-record swap.
    const refused = resolveExteriorActivationSet({ ...base, override: "on", explicitReleaseId: SOUTHERN_REMAINDER_SERVING_RELEASE_ID, activeRealBaseReleaseId: CITYWIDE_BASE, records: rolledBack });
    expect(refused.streaming).toBe(false);
    expect(refused.releases[0]!.reason).toBe("rolled-back-release");
    expect(exteriorRolledBackReleaseNotice(SOUTHERN_REMAINDER_SERVING_RELEASE_ID, rolledBack))
      .toContain(`${SOUTHERN_REMAINDER_SERVING_RELEASE_ID} was rolled back in this build`);
  ...
  it("rolls the Central-and-upper-Manhattan wave back to its curated P1 release without withdrawing the other five", () => {
    ...
    // The withdrawn opt-in link is refused BY NAME, in the same one-record swap.
    const refused = resolveExteriorActivationSet({ ...base, override: "on", explicitReleaseId: CENTRAL_UPPER_MANHATTAN_SERVING_RELEASE_ID, activeRealBaseReleaseId: CITYWIDE_BASE, records: rolledBack });
    expect(refused.streaming).toBe(false);
    expect(refused.releases[0]!.reason).toBe("rolled-back-release");
    expect(exteriorRolledBackReleaseNotice(CENTRAL_UPPER_MANHATTAN_SERVING_RELEASE_ID, rolledBack))
      .toContain(`${CENTRAL_UPPER_MANHATTAN_SERVING_RELEASE_ID} was rolled back in this build`);
  ...
  it("rolls the Northern-Manhattan wave back to its curated P1 release without withdrawing the other five", () => {
    ...
    // The withdrawn opt-in link is refused BY NAME, in the same one-record swap.
    const refused = resolveExteriorActivationSet({ ...base, override: "on", explicitReleaseId: NORTHERN_MANHATTAN_SERVING_RELEASE_ID, activeRealBaseReleaseId: CITYWIDE_BASE, records: rolledBack });
    expect(refused.streaming).toBe(false);
    expect(refused.releases[0]!.reason).toBe("rolled-back-release");
    expect(exteriorRolledBackReleaseNotice(NORTHERN_MANHATTAN_SERVING_RELEASE_ID, rolledBack))
      .toContain(`${NORTHERN_MANHATTAN_SERVING_RELEASE_ID} was rolled back in this build`);
  ...
  it("still names WHICH wave is unavailable when one goes dark all the way to base", () => {
    ...
    const statements = exteriorUnavailableStatements({ set, override: null, activeRealBaseReleaseId: CITYWIDE_BASE, explicitReleaseId: null });
    expect(statements).toHaveLength(1);
    expect(statements[0]).toContain(MIDTOWN_SERVING_RELEASE_ID);
    expect(statements[0]).toContain("not active in this build");
  });
});
