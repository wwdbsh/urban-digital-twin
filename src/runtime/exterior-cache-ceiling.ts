...
export const EXTERIOR_CACHE_EVICTION_DISCLOSURE =
  "Eviction in the shared exterior LRU is RECENCY-ONLY, with no per-wave reservation and no per-wave residency floor; the cache also evicts on the byte cap, which is unchanged at 256 MiB. Raising maxCacheEntries from 256 to 512, and again from 512 to 1024, does not alter that policy and does not repair it — it WIDENS its blast radius, because more promoted waves can now be co-resident and therefore more waves can evict each other's already-verified bytes under camera pressure. At the T005 serving composition the disclosure is WIDER STILL and in a different way: every one of the six promoted releases has content in every cell it owns, so co-residency is no longer occasional but the normal case, and the byte cap now sits at 92.0% of the worst reachable neighbourhood rather than at 48% of the whole promoted composition — which means eviction is no longer a rare backstop but routine, measured at 1.79 evictions per decision on the roaming trace. Nothing renders wrongly when that happens: every evicted artifact is re-fetched and re-verified against its pin before it reaches the scene. The cost is a silent re-fetch that the runtime's published metrics do not attribute to the wave that caused it, and at serving scale that cost is paid far more often. Per-wave residency policy remains deferred against ADR 0024." as const;
