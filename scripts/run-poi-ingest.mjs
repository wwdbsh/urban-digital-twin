...
import { PoiSnapshotAdapter } from "../src/ingestion/poi-snapshot.ts";
...
// The adapter performs the checksum, local-schema, terms, and pending-registry gates
// before this script creates any output directory or file.
const adapter = await PoiSnapshotAdapter.fromSnapshot({
  snapshotText,
  metadata: {
...
const outputs = {
  ...
  layerManifest: `${outputPath}/layer-manifest-pois.json`,
};
...
await writeFile(outputs.features, `${JSON.stringify(await adapter.loadLayerFeatures("pois"), null, 2)}\n`, { encoding: "utf8", flag: "wx" });
...
console.log(JSON.stringify({
  ...
  acceptedPlaces: report.acceptedPlaceCount,
