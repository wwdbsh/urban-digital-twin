"""Blender re-import, measurement and render pass for the Central-and-upper-Manhattan sample.

A sibling of `southern_remainder_sample.py`, which stays exactly as it is and
keeps measuring the byte-frozen Southern-remainder wave and its promoted
successor. Same division of authority: Blender inspects and measures, the Node
writer owns shipped bytes. Nothing here authors geometry — every asset measured
is a GLB that already shipped, opened read-only.

This wave is textured, so the pass carries the whole class of check the
untextured waves had no use for. The earlier untextured passes counted embedded
images only to confirm there were NONE; here the count is a positive assertion,
and it is joined by the two facts that make an embedded image actually
reachable:
...
Check (b) matters more for this wave than for any before it. The wave census
recorded a worst mesh-versus-analytic volume deviation of 0.988 of the writer's
tolerance — the check passed on all 11,543 materialized buildings, but with under
a tenth of the tolerance to spare. This pass recomputes that identity from an
INDEPENDENTLY imported mesh rather than from the writer's own arithmetic, so a
narrow margin produced by a systematic writer error would show up here as a
...
def setup_render():
    ...
    scene.world = bpy.data.worlds.new("udt_central_upper_manhattan_sample_world")
...
def run():
    ...
    report = {
        ...
        "scene": "udt_t019_central_upper_manhattan_sample",
