"""Blender re-import, measurement and render pass for the Lower-Manhattan sample.

A sibling of `midtown_core_v3_sample.py`, which stays exactly as it is and keeps
measuring the byte-frozen Midtown-core V3 wave. Same division of authority:
Blender inspects and measures, the Node writer owns shipped bytes. Nothing here
authors geometry — every asset measured is a GLB that already shipped, opened
read-only.

This is the FIRST TEXTURED WAVE, so the pass carries one whole class of check
the untextured waves had no use for. The earlier passes counted embedded images
...
Renders use the same fixed view and camera convention as the Midtown-core pass,
so the two waves' renders stay comparable by eye.
...
WORK_ROOT = os.path.join(ROOT, "artifacts", "lower-manhattan-20260812", "blender")
...
def setup_render():
    ...
    scene.world = bpy.data.worlds.new("udt_lower_manhattan_sample_world")
...
def run():
    ...
    report = {
        "schemaVersion": "1.0",
        "releaseId": "manhattan-lower-manhattan-cells-20260812",
        "scene": "udt_t015_lower_manhattan_sample",
