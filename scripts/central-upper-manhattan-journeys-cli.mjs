...
/**
 * T019 renderer journeys: the Central-and-upper-Manhattan canary exercised in a
 * real browser against the real pinned citywide base, one journey per claim the
 * canary makes.
 *
 ...
 * the asset on screen, that 246 unshipped cells say so in words, or that pinning
 ...
 * ADDING it: four promoted waves already occupy 434 of the 512 exterior cache
 * entries, so if an opt-in session also held the promoted assets there would be
 * no budget to size against. `canary-opt-in` measures that directly — the
 * promoted releases must fetch ZERO GLBs while the canary fetches all 75.
 *
 * Journeys, in the order they run:
 *   promoted-default-unchanged  A clean no-param load streams the four promoted
 ...
 *   tombstone-truth             The 246 owned cells this release does not
 ...
 *
 * Usage:
 *   node scripts/central-upper-manhattan-journeys-cli.mjs \
...
const capturesRoot = join(repositoryRoot, "artifacts", "central-upper-manhattan-20260812-journeys");
const recordRoot = join(repositoryRoot, "data", "central-upper-manhattan-20260812");
...
/** All FOUR promoted waves. A fifth promotion that missed this list would make
 *  `canary-opt-in` claim isolation it had not measured. */
...
const TRACKED_RELEASE_IDS = [...PROMOTED_RELEASE_IDS, CENTRAL_UPPER_MANHATTAN_RELEASE_ID];
...
function fail(message) { throw new Error(`central-upper-manhattan-journeys: ${message}`); }
 ...
 *
 * THIS WAVE'S CANARY RENDERS THREE CELLS, not the single cell wave w03's did, so
 * the suite has to choose which one to stand in rather than assume there is only
 * one. The rule is stated and derived from committed bytes: the renderable cell
 * that owns the MOST buildings, ties broken by cell id. That is the cell where a
 * facade pose has the most to show, and it is not a preference expressed as a
 * constant. The other two renderable cells are recorded beside it so the record
 * says plainly that the camera stands in one of three.
 */
async function readSubject() {
  const inventory = JSON.parse(await readFile(join(recordRoot, "payload-inventory.json"), "utf8"));
  if (inventory.releaseId !== CENTRAL_UPPER_MANHATTAN_RELEASE_ID) {
    fail(`the committed inventory describes ${inventory.releaseId}, not ${CENTRAL_UPPER_MANHATTAN_RELEASE_ID}.`);
  }
...
async function servedBundleIdentity(previewBase) {
  ...
  if (!new TextDecoder().decode(entryBytes).includes(CENTRAL_UPPER_MANHATTAN_RELEASE_ID)) {
    fail(`the served entry script ${entryPath} does not name ${CENTRAL_UPPER_MANHATTAN_RELEASE_ID}, so it predates the pin and every opt-in journey would fail closed for the wrong reason.`);
  }
...
async function journeyPromotedDefaultUnchanged(port, previewBase, subject, pose) {
  ...
  try {
    ...
    return {
      ...
      claim: "Pinning this canary changed nothing about what an ordinary session loads. A clean load with no exterior URL parameter still streams the four promoted waves and fetches ZERO bytes of the Central-and-upper-Manhattan release, even with the camera standing inside its framed renderable cell — a pose derived from that cell's committed ledger bounds and asserted to lie within them before any capture.",
      url,
      waves,
      network,
      passed: PROMOTED_RELEASE_IDS.every((releaseId) => network.perRelease[releaseId].glbCount > 0)
        ...
        && network.perRelease[CENTRAL_UPPER_MANHATTAN_RELEASE_ID].encodedBytes === 0
...
async function journeyCanaryOptIn(port, previewBase, subject, pose, promotedDefaultStill) {
  const session = await openFreshPage(port);
  try {
    const url = appUrl(previewBase, { pose, params: { exteriorCells: CENTRAL_UPPER_MANHATTAN_RELEASE_ID, exteriorStreaming: "on" } });
    session.events.length = 0;
    await session.send("Page.navigate", { url });
    const waves = await waitFor(
      session,
    ...
    await settleGlbCount(session, CENTRAL_UPPER_MANHATTAN_RELEASE_ID, subject.assetCount);
    const network = networkPerRelease(session);
    ...
    return {
      ...
      claim: "`?exteriorCells=` SELECTS the named release rather than adding it: the opt-in link streams all of the Central-and-upper-Manhattan canary's textured assets and ZERO GLBs of any of the four promoted waves. This is the measurement the canary's entry budget rests on — the promoted waves occupy 434 of 512 cache entries, so an opt-in session that also held them would have no budget to size against. The still must also DIFFER from the promoted default's still at the identical pose, so the assets are shown to be drawn rather than merely fetched.",
      url,
      waves,
      network,
      ...
      passed: network.perRelease[CENTRAL_UPPER_MANHATTAN_RELEASE_ID].glbCount === subject.assetCount
  ...
  try {
    ...
    return {
      ...
      passed: Boolean(detail)
        && String(detail.badge ?? "").includes(CENTRAL_UPPER_MANHATTAN_RELEASE_ID)
...
async function journeyTombstoneTruth(port, previewBase, subject, pose) {
  ...
  try {
    ...
    const waveNotice = notices.find((notice) => notice.includes(CENTRAL_UPPER_MANHATTAN_RELEASE_ID)) ?? null;
    return {
      journeyId: "tombstone-truth",
      claim: `The ${subject.tombstonedCellCount} owned w04 cells this release does NOT materialize are reported in words — the release says how many of its ${subject.ownedCellCount} cells ship no generated exterior geometry, and what draws for them instead — rather than going quiet.`,
      url,
      notices,
      waveNotice,
...
async function main() {
  ...
  // being served, WHAT this release shipped, WHICH of its three renderable cells
  // is framed, and WHERE the camera stands. Each of them fails the run rather than
  // being recorded as a caveat.
  ...
  const tombstoneTruth = await journeyTombstoneTruth(port, previewBase, subject, pose);
  const journeys = [promotedDefault, canaryOptIn, texturedPick, tombstoneTruth];

  const evidence = {
    schemaVersion: "1.0",
    releaseId: CENTRAL_UPPER_MANHATTAN_RELEASE_ID,
    note: "T019 renderer journeys against the production preview and the real pinned citywide base. Each entry records the claim it tests, the URL it used, the DOM text it read, the per-release network measurement, and a checksummed still. `passed` is computed from the readings, not asserted.",
    capturedWith: {
      viewport: VIEWPORT,
      previewBase,
    ...
    notMeasuredHere: [
      "Frame time, heap, GPU texture accounting and cache residency. Those are PROMOTION's instrument: they measure a promoted composition under the default activation, and this release is not promoted. The tile system's own cost was measured by T015's kill switch, re-measured off the vsync floor by T016, and re-measured again over a four-wave composition at the raised cache cap by T018; this wave changes which buildings carry those tiles, not the tiles.",
      "The two renderable cells the camera does not stand in. The framed cell is the one owning the most buildings; the other two ship and are streamed by the opt-in journey's asset count, but no still is taken from inside them and none is claimed.",
