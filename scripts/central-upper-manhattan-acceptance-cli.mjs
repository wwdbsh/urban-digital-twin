...
/**
 * T020 promotion acceptance capture: the FIVE-WAVE PROMOTED COMPOSITION measured
 * in the production preview, OFF THE VSYNC FLOOR, AT THE UNCHANGED CACHE CAP.
 *
 * A sibling of `scripts/southern-remainder-acceptance-cli.mjs`, which stays
 * exactly as it is and keeps describing the four-wave composition it measured.
 * Three things differ, and each is a requirement rather than a preference:
 *
 *   1. THE CAP DID NOT MOVE, and that is itself the thing to record. ADR 0036
 *      precondition (a) admitted a further raise as one of three responses and
 *      T020 took response 2 instead, so `capAtMeasurement` states that 512 was
 *      in force and that this promotion did not change it. The value is read
 *      from `EXTERIOR_RUNTIME_BUDGETS` in the tree the served bundle was built
 *      from rather than typed into this record.
 ...
 *      unchanged in substance and retargeted at the T020 P1 release, because an
 *      acceptance measurement against the wrong bundle is worse than none.
 *   3. FIVE RELEASES ARE ATTRIBUTED, not four. A FIVE-wave composition had never
 *      been measured: ADR 0036 precondition (d) says so in terms, and says that
 *      T018's readings are for four waves at 420 worst-observed residency
 *      against a budgeted 434, both of which a fifth wave moves.
 *
 ...
 *
 * Usage:
 *   node scripts/central-upper-manhattan-acceptance-cli.mjs \
 *     --preview http://localhost:4174 --port 9222 [--repeats 3] [--capped-port 9333] [--out <path>]
 */
...
const capturesRoot = join(repositoryRoot, "artifacts", "central-upper-manhattan-20260812-p1-acceptance", "captures");
const defaultOutPath = join(repositoryRoot, "data", "central-upper-manhattan-20260812-p1", "acceptance-evidence.json");
const distRoot = join(repositoryRoot, "dist");
const p1AssetsRoot = join(repositoryRoot, "public", "data", CENTRAL_UPPER_MANHATTAN_P1_RELEASE_ID, "public", "assets");
...
function fail(message) { throw new Error(`central-upper-manhattan-acceptance: ${message}`); }
...
const STATIONS = [
  {
    ...
    profile: "inspection",
    note: "Standing on the PROMOTED PARKLAND of curated cell 491 and looking WEST at the tower wall of curated cell 490, on the inspection profile at a shallow pitch. Shallow and low on purpose: facades are vertical, and the T017 evidence records a station that looked at rooftops and produced byte-identical stills. This is the pose the ground-coverage tie-break was decided for — the viewer is inside the subset looking across it — and the textured LOD 0 wall stands directly beside the untextured base massing of the cells this release tombstones.",
    pose: { lon: -73.97400, lat: 40.77160, height: 150, heading: 272, pitch: -4, roll: 0 },
  },
  {
    stationId: "centralpark-west-skyline",
    profile: "exploration",
    note: "The wide skyline station the curation was chosen for: the whole curated wall read as one silhouette from the south-east, including the tallest sourced structure in any admissible cell of this wave at 219.2 m, past the LOD 0 cut-off where a 128-pixel tile is repeated most per screen pixel and aliasing shows first.",
    pose: { lon: -73.97000, lat: 40.76500, height: 520, heading: 315, pitch: -8, roll: 0 },
  },
  {
    stationId: "fivewave-wide",
    profile: "exploration",
    note: "The FIVE-WAVE wide view, and the station this promotion exists to measure. A high pose south of the island looking north up Manhattan, holding the promoted Lower-Manhattan, Southern-remainder, Block 835, Midtown-core and Central-and-upper-Manhattan waves in one frame. ADR 0036 precondition (d) states that a five-wave composition had never been measured; this is the pose where five co-resident waves are simultaneously in view rather than merely simultaneously promoted.",
    pose: { lon: -74.01200, lat: 40.69800, height: 4200, heading: 22, pitch: -22, roll: 0 },
  },
  {
    stationId: "nomad-facade",
    profile: "inspection",
    note: "The T018 Southern-remainder station, pose-for-pose UNCHANGED, kept for continuity across the two promotions. Its numbers are directly comparable with the T018 record's, so \"did adding a fifth wave cost the fourth one anything\" is a comparison rather than an assertion.",
...
async function captureStation(port, previewBase, station, repeatIndex, extraParams) {
  ...
  try {
    ...
    const perRelease = {};
    for (const releaseId of [...PROMOTED_RELEASE_IDS, CENTRAL_UPPER_MANHATTAN_RELEASE_ID, BASE_RELEASE_ID]) {
...
async function gpuTextureArithmetic() {
  ...
  return {
    ...
    otherPromotedWavesNote: "The Block 835 V3 and Midtown-core V3 waves are TEXTURE-FREE releases and contribute nothing. The Lower-Manhattan P1 and Southern-remainder P1 waves are NOT: both are textured LOD 0, over 71 and 179 assets, and each computes its own share in its own T016 and T018 record. The figure above is THIS release's share alone and must not be read as the whole promoted composition's; `promotedCompositionNote` states how the sum is formed.",
    promotedCompositionNote: "Three of the five promoted waves carry tiles. Adding this release's computed share to the T016 and T018 records' gives the promoted composition's computed upper bound; all three are arithmetic and none is a measurement.",
  };
}
...
async function main() {
  ...
  const cacheResidency = {
    ...
    releaseTimeDerivation: {
      maxCacheEntries: EXTERIOR_RUNTIME_BUDGETS.maxCacheEntries,
      promotedAssetEntriesBeforeThisWave: 434,
      thisWaveAssetEntries: 40,
      residentAssetEntries: 474,
      headroomForFurtherWaves: EXTERIOR_RUNTIME_BUDGETS.maxCacheEntries - 474,
      reservedForNorthernManhattanEntries: 36,
      note: "The RELEASE-TIME derivation counts GLB FILES ON DISK: 28 for Block 835 V3, which ships two LODs per building, 156 for Midtown-core V3, 71 for Lower-Manhattan P1, 179 for Southern-remainder P1 and 40 for this release. At runtime a session fetches fewer, because only the selected LOD is requested — the T016 record measured 14 Block 835 GLBs against 28 on disk — so the disk derivation is CONSERVATIVE rather than wrong, and `worstObserved` above is what this session actually made resident. An immutable release must be sized against the release-time number, which is why both are recorded. ADR 0034's admissible response 3 — counting RESOLVED entries rather than files on disk — was considered at T020 and REJECTED: both canonical levels of detail of Block 835 can be resident in one session under ordinary camera movement, so a resolved count would understate real occupancy by exactly the amount that matters. The 38 entries left are two more than the 36 this promotion's split RESERVED for wave w05, so a T022 promotion that fits its reservation fits this cache without another cap change.",
    },
  };
  ...
  const evidence = {
    ...
    capAtMeasurement: {
      maxCacheEntries: EXTERIOR_RUNTIME_BUDGETS.maxCacheEntries,
      maxCachedBytes: EXTERIOR_RUNTIME_BUDGETS.maxCachedBytes,
      statement: "THE 512-ENTRY CAP WAS IN FORCE FOR EVERY READING BELOW AND THIS PROMOTION DID NOT CHANGE IT. ADR 0036 precondition (a) admitted a further raise as one of three responses; T020 took response 2 — split the 78-entry headroom with the one wave that is still unpromoted — so both cache caps are exactly what T018 left them. The values here are read from EXTERIOR_RUNTIME_BUDGETS in the same tree the served bundle was built from rather than typed into this record, so a reading taken at a different cap cannot be presented as a reading at this one.",
    },
    note: "T020 promotion acceptance. The PROMOTED DEFAULT COMPOSITION — Block 835 V3, Midtown-core V3, the Lower-Manhattan P1 successor, the Southern-remainder P1 successor and the Central-and-upper-Manhattan P1 successor streaming together over the pinned manhattan-citywide-20260804 base — measured in the production preview build by the shipping CesiumJS renderer, with Chrome's vsync and frame-rate limit disabled, at the UNCHANGED 512-entry cache cap. A FIVE-wave composition had never been measured before this run, which is what ADR 0036 precondition (d) required. That precondition also asked for the capped control to be captured in a second browser, for GPU texture memory to be labelled COMPUTED if it is computed, and for isolated slow frames to be STATED rather than smoothed: `cappedControl` is the reading that proves this run is uncapped, `gpuTextureArithmetic` is computed and is not measured anywhere in this file, and every station carries `worstObservedFrameMilliseconds` beside its medians.",
