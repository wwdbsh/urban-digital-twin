...
/**
 * Operator entrypoint for the Block 835 generated-exterior reference package.
 *
 * Commands
...
const PLAN_DIR = join(ROOT, "data", BLOCK835_REFERENCE_PACKAGE_ID, "plans");
const MEASUREMENT_PATH = join(ROOT, "data", BLOCK835_REFERENCE_PACKAGE_ID, "silhouette-measurements.json");
const PACKAGE_DIR = join(ROOT, "public", "data", BLOCK835_REFERENCE_PACKAGE_ID);
...
async function loadMeasurements(path = MEASUREMENT_PATH) {
  ...
  if (parsed.packageId !== BLOCK835_REFERENCE_PACKAGE_ID || parsed.method !== "projected-silhouette-ratio" || parsed.metricVersion !== "1.0") fail("Silhouette measurement file does not match the approved package/metric identity.");
...
async function writePlans() {
  ...
  await writeFile(join(dirname(PLAN_DIR), "plan-index.json"), `${stableSerialize({ packageId: BLOCK835_REFERENCE_PACKAGE_ID, plans: summary })}\n`, "utf8");
...
async function authoringInputs(targetDir) {
  ...
  for (const building of readPilotBuildings(release)) {
    ...
    const entry = {
      ...
      tessellation: { openingInsetMm: context.plan.input.parameters.openingInsetMm, roofEquipmentHeightMm: roofEquipmentHeightMm(context.plan) },
  ...
  await writeFile(join(targetDir, "index.json"), `${stableSerialize({ packageId: BLOCK835_REFERENCE_PACKAGE_ID, generatedAt: BLOCK835_REFERENCE_GENERATED_AT, buildings: index })}\n`, "utf8");
...
async function promoteMeasurements(evidencePath) {
  ...
  if (evidence.packageId !== BLOCK835_REFERENCE_PACKAGE_ID || evidence.method !== "projected-silhouette-ratio" || evidence.metricVersion !== "1.0") fail("Blender silhouette evidence does not match the approved package/metric identity.");
  const { release } = await loadPilot();
  ...
  const file = {
    ...
    measuredAt: BLOCK835_REFERENCE_GENERATED_AT,
    buildings,
  };
