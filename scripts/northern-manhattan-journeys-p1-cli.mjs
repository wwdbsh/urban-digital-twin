...
/**
 * T022 renderer journeys: the SIX-WAVE promoted composition — every wave the
 * committed ledger declares — exercised in a real browser against the real base,
 * one journey per claim the promotion makes.
 *
 * A promotion record and a drift gate prove what the BUILD accepts. They cannot
 * prove that a clean session streams six waves, that a picked Harlem building
 * names its own release and checksum, that an unshipped cell says so in words, or
 * that switching exteriors off switches all six off. Those are browser facts, and
 * this captures them as browser facts.
 *
 ...
 *   shared port was measured once and its "pass" was vacuous. `servedBundle`
 ...
 *   that proves the 24 curated assets reach the screen.
 *
 ...
 *   cold-default        A clean no-param load streams all SIX waves, and the
 *                       curated w05 cell renders its 24 textured assets. Every
 *                       wave's GLB count is NAMED in the record rather than
 *                       summarised, because "six waves streamed" is the claim
 *                       this promotion exists to make.
 *   cross-wave-pick     The curated w05 cell's tallest building names its
 *                       release, cell, cell release, asset checksum, truth tiers
 *                       and uncertainty statement.
 *   canary-opt-in       The T021 canary's `?exteriorCells=` link still resolves to
 *                       the canary alone, unchanged by its wave's promotion.
 *   streaming-off       `exteriorStreaming=off` disables ALL six waves, and its
 *                       still DIFFERS from the promoted default's at the same
 *                       pose, which is what proves the tiles are drawn.
 *   tombstone-truth     An unshipped w05 cell states that it ships no exterior
 ...
 *
 * Usage:
 *   node scripts/northern-manhattan-journeys-p1-cli.mjs \
...
const capturesRoot = join(repositoryRoot, "artifacts", "northern-manhattan-20260812-p1-journeys", "journeys");
const defaultOutPath = join(repositoryRoot, "data", "northern-manhattan-20260812-p1", "journey-evidence.json");
...
const PROMOTED_ASSET_COUNT = 24;
const CANARY_ASSET_COUNT = 76;
const CANARY_CELL_ID = "manhattan-exterior-cell-w05-000701-15-9651-8954";
/** The tallest sourced structure in any admissible cell of wave w05, at 101.5 m. */
const SKYLINE_BUILDING_ID = "doitt:342401";
...
/**
 * A camera EAST of curated cell 727 looking WEST along the West 125th Street
 * corridor; the same pose the acceptance run measured at `harlem-125th-facade`.
 * Low and shallow on purpose: facades are vertical, and T017 records a corrected
 * pose that stood 260 m up looking down and produced byte-identical stills.
 */
const CURATED_POSE = { lon: -73.94550, lat: 40.80940, height: 150, heading: 270, pitch: -4, roll: 0 };

function fail(message) { throw new Error(`northern-manhattan-journeys-p1: ${message}`); }
...
async function journeyColdDefault(port, previewBase) {
  ...
  try {
    ...
    // count the instant the wave went active and read 7 of 71. Wait for the w05
    ...
    for (let attempt = 0; attempt < 90; attempt += 1) {
      const count = networkPerRelease(session).perRelease[NORTHERN_MANHATTAN_P1_RELEASE_ID].distinctGlbCount;
    ...
    // EVERY WAVE'S GLB COUNT, NAMED. "Six waves streamed" is the claim this
    // promotion exists to make, and a summary that only asserted "> 0 each" would
    // let a wave that delivered one asset stand for a wave that delivered all of
    // them. The exact count is asserted only for THIS release, because the others
    // fetch the LOD the camera selected rather than everything on disk.
    const perWaveGlbCounts = Object.fromEntries(PROMOTED_RELEASE_IDS.map((releaseId) => [releaseId, {
      responses: network.perRelease[releaseId].glbCount,
      distinctArtifacts: network.perRelease[releaseId].distinctGlbCount,
    }]));
    return {
      ...
      network,
      perWaveGlbCounts,
      passed: Object.keys(perWaveGlbCounts).length === 6
        && network.perRelease[NORTHERN_MANHATTAN_P1_RELEASE_ID].distinctGlbCount === PROMOTED_ASSET_COUNT
        && network.perRelease[NORTHERN_MANHATTAN_RELEASE_ID].glbCount === 0
        && PROMOTED_RELEASE_IDS.every((releaseId) => network.perRelease[releaseId].distinctGlbCount > 0)
...
async function journeyCrossWavePick(port, previewBase) {
  ...
  try {
    ...
    return {
      ...
      claim: "A picked w05 building — the tallest sourced structure in any cell this wave could have promoted inside its 36-entry reservation, at a sourced 101.5 m — names the release it came from, its cell and cell release, the checksum of the exact asset on screen, its generated truth tiers, and an uncertainty statement. The composition holds SIX waves at once, so the pick also proves the details panel attributes to the RIGHT wave.",
      url,
      buildingId: SKYLINE_BUILDING_ID,
      searchResults: results,
      detail,
      passed: Boolean(detail)
        && String(detail.badge ?? "").includes(NORTHERN_MANHATTAN_P1_RELEASE_ID)
        && String(rows["Cell / release"] ?? "").includes("manhattan-exterior-cell-w05-")
...
async function journeyCanaryOptIn(port, previewBase, canary) {
  const session = await openFreshPage(port);
  try {
    const url = appUrl(previewBase, { pose: canary.pose, params: { exteriorCells: NORTHERN_MANHATTAN_RELEASE_ID, exteriorStreaming: "on" } });
    session.events.length = 0;
    await session.send("Page.navigate", { url });
    const waves = await waitFor(
      session,
    ...
    for (let attempt = 0; attempt < 90; attempt += 1) {
      const count = networkPerRelease(session).perRelease[NORTHERN_MANHATTAN_RELEASE_ID].distinctGlbCount;
    ...
    return {
      ...
      claim: "The T021 canary's opt-in link still resolves to the canary ALONE, from a camera derived from that canary cell's own committed bounds. Promoting the wave's P1 successor did not withdraw the canary, and `?exteriorCells=` still means exactly the release it names — including excluding the successor that supersedes it, and including excluding the five other promoted waves.",
      url,
      poseContainment: canary.containment,
      waves,
      network,
      // Every promoted wave fetches NOTHING: `?exteriorCells=` means exactly the
      // release it names, so an opt-in replaces the whole six-wave default set
      // rather than adding to it.
      passed: network.perRelease[NORTHERN_MANHATTAN_RELEASE_ID].distinctGlbCount === CANARY_ASSET_COUNT
...
async function journeyStreamingOff(port, previewBase, promotedDefaultStill) {
  ...
  try {
    ...
    // still equalled the promoted default's, the 24 curated assets would have
    ...
    return {
      ...
      claim: "`exteriorStreaming=off` disables ALL SIX promoted waves, not just the newest, no exterior GLB is fetched at all, and the resulting still DIFFERS from the promoted default's at the identical pose — which is what proves the curated w05 assets are drawn rather than merely downloaded.",
      url,
      waves,
...
async function journeyTombstoneTruth(port, previewBase) {
  ...
  try {
    ...
    const waveNotice = notices.find((notice) => notice.includes(NORTHERN_MANHATTAN_P1_RELEASE_ID)) ?? null;
    return {
      journeyId: "tombstone-truth",
      claim: "The 181 owned w05 cells this release does NOT materialize are reported in words — the release says how many of its cells ship no generated exterior geometry, and what draws for them instead — rather than going quiet.",
      url,
      notices,
      waveNotice,
      passed: typeof waveNotice === "string"
        && waveNotice.includes("181 of 182")
...
async function main() {
  ...
  const evidence = {
    ...
    note: "T022 renderer journeys against the production preview and the real pinned citywide base, with the served bundle identified before any capture. Each entry records the claim it tests, the URL it used, the DOM text it read, the per-release network measurement, and a checksummed still. `passed` is computed from the readings, not asserted. THE COMPOSITION UNDER TEST IS THE WHOLE LEDGER: six declared waves, six promoted defaults, and `cold-default.perWaveGlbCounts` names what each one actually delivered rather than summarising them into a pass.",
    servedBundle,
    ...
    coveredByTestsInstead: [
      "Per-wave ROLLBACK rehearsal: no URL expresses a build-time promotion-record swap, so it is proven through the record's own injection seam in src/runtime/exterior-multiwave-activation.test.ts ('rolls the Northern-Manhattan wave back to BASE MASSING without withdrawing the other five') — which also checks that the withdrawn successor's link is refused BY NAME while the T021 canary's opt-in and the other five waves' links are untouched, and that five of six waves keep streaming, which is what un-completing the ledger's coverage looks like in practice.",
