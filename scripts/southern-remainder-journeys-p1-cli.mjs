...
/**
 * T018 renderer journeys: the FOUR-WAVE promoted composition exercised in a real
 ...
 * prove that a clean session streams four waves, that a picked NoMad building
 * names its own release and checksum, that an unshipped cell says so in words, or
 * that switching exteriors off switches all four off. Those are browser facts,
 * and this captures them as browser facts.
 *
 ...
 *   that proves the 179 curated assets reach the screen.
 *
 ...
 *   cold-default        A clean no-param load streams all FOUR waves, and the
 *                       curated w03 cells render their 179 textured assets.
 *   cross-wave-pick     A curated w03 skyline building names its release, cell,
 *                       cell release, asset checksum, truth tiers and
 *                       uncertainty statement.
 *   canary-opt-in       The T017 canary's `?exteriorCells=` link still resolves
 *                       to the canary alone, unchanged by its wave's promotion.
 *   streaming-off       `exteriorStreaming=off` disables ALL four waves, and its
 *                       still DIFFERS from the promoted default's at the same
 *                       pose, which is what proves the tiles are drawn.
 *   tombstone-truth     An unshipped w03 cell states that it ships no exterior
 ...
 *
 * Usage:
 *   node scripts/southern-remainder-journeys-p1-cli.mjs \
...
const capturesRoot = join(repositoryRoot, "artifacts", "southern-remainder-20260812-p1-journeys", "journeys");
const defaultOutPath = join(repositoryRoot, "data", "southern-remainder-20260812-p1", "journey-evidence.json");
...
const PROMOTED_ASSET_COUNT = 179;
const CANARY_ASSET_COUNT = 76;
const CANARY_CELL_ID = "manhattan-exterior-cell-w03-000276-17-38590-35872";
/** The tallest sourced structure in the whole of wave w03, at 245.4 m. */
const SKYLINE_BUILDING_ID = "doitt:1290754";
...
/**
 * A camera inside the curated w03 block; the same pose the acceptance run
 * measured at `nomad-facade`. Low and shallow on purpose: facades are vertical,
 * and T017 records a corrected pose that stood 260 m up looking down and produced
 * byte-identical stills.
 */
const NOMAD_POSE = { lon: -73.99250, lat: 40.74580, height: 150, heading: 65, pitch: -4, roll: 0 };

function fail(message) { throw new Error(`southern-remainder-journeys-p1: ${message}`); }
...
function networkPerRelease(session) {
  ...
  const perRelease = {};
  for (const releaseId of [...PROMOTED_RELEASE_IDS, SOUTHERN_REMAINDER_RELEASE_ID]) {
...
async function journeyColdDefault(port, previewBase) {
  ...
  try {
    const url = appUrl(previewBase, { pose: NOMAD_POSE });
    ...
    // count the instant the wave went active and read 7 of 71. Wait for the w03
    ...
    for (let attempt = 0; attempt < 90; attempt += 1) {
      const count = networkPerRelease(session).perRelease[SOUTHERN_REMAINDER_P1_RELEASE_ID].glbCount;
    ...
    return {
      ...
      claim: "A clean load with no exterior URL parameter streams all FOUR promoted waves over the real citywide base, and the curated w03 cells render their 179 textured assets. The T017 canary is NOT loaded, because it is not promoted.",
      url,
      waves,
      network,
      passed: network.perRelease[SOUTHERN_REMAINDER_P1_RELEASE_ID].glbCount === PROMOTED_ASSET_COUNT
...
async function journeyCrossWavePick(port, previewBase) {
  ...
  try {
    ...
    return {
      ...
      claim: "A picked w03 skyline building — the tallest sourced structure in the wave — names the release it came from, its cell and cell release, the checksum of the exact asset on screen, its generated truth tiers, and an uncertainty statement. The composition holds four waves at once, so the pick also proves the details panel attributes to the RIGHT wave.",
      url,
      buildingId: SKYLINE_BUILDING_ID,
      searchResults: results,
      detail,
      passed: Boolean(detail)
        && String(detail.badge ?? "").includes(SOUTHERN_REMAINDER_P1_RELEASE_ID)
        && String(rows["Cell / release"] ?? "").includes("manhattan-exterior-cell-w03-")
...
async function journeyCanaryOptIn(port, previewBase, canary) {
  const session = await openFreshPage(port);
  try {
    const url = appUrl(previewBase, { pose: canary.pose, params: { exteriorCells: SOUTHERN_REMAINDER_RELEASE_ID, exteriorStreaming: "on" } });
    session.events.length = 0;
    await session.send("Page.navigate", { url });
    const waves = await waitFor(
      session,
    ...
    for (let attempt = 0; attempt < 90; attempt += 1) {
      const count = networkPerRelease(session).perRelease[SOUTHERN_REMAINDER_RELEASE_ID].glbCount;
...
async function journeyStreamingOff(port, previewBase, promotedDefaultStill) {
  const session = await openFreshPage(port);
  try {
    const url = appUrl(previewBase, { pose: NOMAD_POSE, params: { exteriorStreaming: "off" } });
    session.events.length = 0;
    await session.send("Page.navigate", { url });
    ...
    // still equalled the promoted default's, the 179 curated assets would have
    ...
    return {
      ...
      claim: "`exteriorStreaming=off` disables ALL FOUR promoted waves, not just the newest, no exterior GLB is fetched at all, and the resulting still DIFFERS from the promoted default's at the identical pose — which is what proves the curated w03 assets are drawn rather than merely downloaded.",
      url,
      waves,
...
async function journeyTombstoneTruth(port, previewBase) {
  ...
  try {
    ...
    const waveNotice = notices.find((notice) => notice.includes(SOUTHERN_REMAINDER_P1_RELEASE_ID)) ?? null;
    return {
      journeyId: "tombstone-truth",
      claim: "The 172 owned w03 cells this release does NOT materialize are reported in words — the release says how many of its cells ship no generated exterior geometry, and what draws for them instead — rather than going quiet.",
      url,
      notices,
      waveNotice,
      passed: typeof waveNotice === "string"
        && waveNotice.includes("172 of 176")
...
async function main() {
  ...
  const evidence = {
    ...
    coveredByTestsInstead: [
      "Per-wave ROLLBACK rehearsal: no URL expresses a build-time promotion-record swap, so it is proven through the record's own injection seam in src/runtime/exterior-multiwave-activation.test.ts ('rolls the Southern-remainder wave back to BASE MASSING without withdrawing the other three') — which also checks that the withdrawn successor's link is refused BY NAME while the T017 canary's opt-in and the other three waves' links are untouched.",
